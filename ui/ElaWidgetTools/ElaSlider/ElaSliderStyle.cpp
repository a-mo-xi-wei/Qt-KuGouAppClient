...
ElaSliderStyle::~ElaSliderStyle() = default;
    ...
    case QStyle::CC_Slider:
    {
        ...
        if (sopt->orientation == Qt::Horizontal)
        {
            ...
            painter->drawRoundedRect(
                QRect(sliderRect.x() + sliderRect.height() / 8, sliderRect.y() + sliderRect.height() * 0.375,
                      sliderRect.width() - sliderRect.height() / 4, sliderRect.height() / 4), sliderRect.height() / 8,
                sliderRect.height() / 8);
            // 已滑过
            // 已滑过
            QLinearGradient gradient;
            gradient.setStart(sliderRect.left(), 0);
            gradient.setFinalStop(sliderHandleRect.center().x(), 0);
            gradient.setColorAt(0.0, QColor("pink"));
            gradient.setColorAt(1.0, QColor(0xB0EDF6));
            painter->setBrush(gradient);

            painter->drawRoundedRect(
                QRect(sliderRect.x() + sliderRect.height() / 8, sliderRect.y() + sliderRect.height() * 0.375,
                      sliderHandleRect.x(), sliderRect.height() / 4), sliderRect.height() / 8, sliderRect.height() / 8);
        ...
        else
        {
            // 未滑过
            painter->drawRoundedRect(
                QRect(sliderRect.x() + sliderRect.width() * 0.375, sliderRect.y() + sliderRect.width() / 8,
                      sliderRect.width() / 4, sliderRect.height() - sliderRect.width() / 4), sliderRect.width() / 8,
                sliderRect.width() / 8);
            // 已滑过
            painter->setBrush(ElaThemeColor(_themeMode, PrimaryNormal));
            painter->drawRoundedRect(
                QRect(sliderRect.x() + sliderRect.width() * 0.375, sliderHandleRect.y(), sliderRect.width() / 4,
                      sliderRect.height() - sliderRect.width() / 8 - sliderHandleRect.y()), sliderRect.width() / 8,
                sliderRect.width() / 8);
        }
        // 滑块
        // 外圆形
        ...
        if (sopt->activeSubControls == SC_SliderHandle)
        {
            ...
            else
            {
                if (sopt->state & QStyle::State_MouseOver)
                {
                    ...
                    {
                        _startRadiusAnimation(_circleRadius, sliderHandleRect.width() / 2.8,
                                              const_cast<QWidget*>(widget));
                        _lastState = sopt->state;
                    }
                    if (_lastState.testFlag(QStyle::State_Sunken))
