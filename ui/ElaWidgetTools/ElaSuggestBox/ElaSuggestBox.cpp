...
ElaSuggestBox::ElaSuggestBox(QWidget *parent) : QWidget{parent}, d_ptr(new ElaSuggestBoxPrivate())
{
    ...
    d->_darkSearchAction = new QAction(
        ElaIcon::getInstance()->getElaIcon(ElaIconType::MagnifyingGlass, QColor(0xFF, 0xFF, 0xFF)), "Search",
        this); ///< 暗色搜索图标

    d->_themeMode = eTheme->getThemeMode(); ///< 获取当前主题模式
    connect(eTheme, &ElaTheme::themeModeChanged, d,
            &ElaSuggestBoxPrivate::onThemeModeChanged); ///< 连接主题变化信号
    // 获取关联的QToolButton
    ...
    connect(d->_searchView, &ElaBaseListView::clicked, d,
            &ElaSuggestBoxPrivate::onSearchViewClicked); ///< 连接点击信号
    connect(d->_searchEdit, &ElaLineEdit::textChanged, d,
            &ElaSuggestBoxPrivate::onSearchEditTextEdit); ///< 连接文本变化信号
...
void ElaSuggestBox::setLineEdit(ElaLineEdit *lineEdit)
{
    ...
    if (d->_themeMode == ElaThemeType::Light)
    {
        ...
        connect(d->_lightSearchAction, &QAction::triggered, d, [d, this] {
            d->_lightSearchAction->setEnabled(false);
            d->_searchEdit->clearFocus();
            QTimer::singleShot(0, this, [d] {
                d->_startCloseAnimation(); ///< 关闭建议框
            });
    ...
    else
    {
        ...
        connect(d->_darkSearchAction, &QAction::triggered, d, [d, this] {
            d->_darkSearchAction->setEnabled(false);
            d->_searchEdit->clearFocus();
            QTimer::singleShot(0, this, [d] {
                d->_startCloseAnimation(); ///< 关闭建议框
            });
    ...
    connect(d->_searchEdit, &ElaLineEdit::wmFocusOut, this, [d] {
        ...
        // @note 延迟执行关闭动画
        QTimer::singleShot(300, d, [d] { d->_startCloseAnimation(); });
    });
...
void ElaSuggestBox::setSearchEnable(bool enable)
{
    Q_D(ElaSuggestBox);

    d->_searchEdit->setEnabled(enable);
    if (d->_themeMode == ElaThemeType::Light)
    {
        d->_lightSearchAction->setEnabled(enable);
    }
    else
    {
        d->_darkSearchAction->setEnabled(enable);
    }
}
