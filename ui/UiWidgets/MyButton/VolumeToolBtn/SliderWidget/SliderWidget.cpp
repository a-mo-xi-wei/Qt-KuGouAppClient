...
void SliderWidget::handleDelta(const int &delta)
{
    if (delta > 0) {
        ...
        this->m_currentValue = this->m_currentValue >= this->m_maxValue
                                   ? this->m_maxValue
                                   : this->m_currentValue;
    } else if (delta < 0) {
        this->m_currentValue -= 2;
        this->m_currentValue = this->m_currentValue < this->m_minValue
                                   ? this->m_minValue
                                   : this->m_currentValue;
    }
...
void SliderWidget::mousePressEvent(QMouseEvent *event)
{
    if (orientation() == Qt::Horizontal) {
        ...
        // 根据百分比计算出滑动条的新值
        this->m_currentValue = this->m_minValue + static_cast<int>(
                                   percentage * (this->m_maxValue - this->m_minValue));
...
void SliderWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    // 根据百分比计算出滑动条的新值
    auto moveValue = this->m_minValue + static_cast<int>(
                         percentage * (this->m_maxValue - this->m_minValue));
    // 设置滑动条的新值
    //判断区间
