...
void DynamicBackgroundGradient::draw(QPainter &painter, const QPainterPath &path)
{
    painter.save();
    painter.setRenderHint(QPainter::Antialiasing, true);

    // 获取路径的boundingRect作为渐变区域
    QRect rect = path.boundingRect().toRect();

    QPoint pu, pd;
    if (!horizone) {
        pu = QPoint(rect.left() + rect.width() / 2, rect.top());
        pd = QPoint(rect.left() + rect.width() / 2, rect.bottom());
    } else {
        pu = QPoint(rect.left(), rect.top() + rect.height() / 2);
        pd = QPoint(rect.right(), rect.top() + rect.height() / 2);
    }
    ...
    painter.drawPath(path); // ⚠️ 用路径填充而不是矩形
