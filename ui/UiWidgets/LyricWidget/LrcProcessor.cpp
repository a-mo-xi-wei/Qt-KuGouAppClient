...
bool LrcProcessor::LoadFromFile(const QString& lyricFilePath)
{
    lrcLyrics.clear();
    ...
    while (!file.atEnd()) {
        ...
        if (strline.size() != 0)
            lines.emplace_back(strline); //收集非空行
    }
...
bool LrcProcessor::LoadFromRawText(const QString& content)
{
...
void LrcProcessor::LoadFromRawLines(const QVector<QString>& lines)
{
    //进一步判断是否为LRC歌词
    if (lines.size() != 0) {
        bIsLyricValid = true;
        ...
        for (auto &line : lines) {
            ...
            for (auto &time : timeList) //只有当收集到时间标签时，才收集对应的歌词进入 lrcLyricsTemp
            {
                ...
                lrcLyricsTemp.emplace_back(QPair<int, QString>(msec, strLeft.trimmed()));
            }
        ...
        if (lrcLyricsTemp.size() != 0) {
            ...
            while (lrcLyricsTemp.size() != 0) {
                ...
                lrcLyrics.emplace_back(*iterDel);
            ...
            for (auto &line : lines) {
                lrcLyrics.emplace_back(QPair<int, QString>(0, line.trimmed()));
            }
        }
    }
}
...
QString LrcProcessor::ToLrcLine(QPair<int, QString> timeLine)
{
    ...
    QString timeLabel = QString::asprintf("[%.2d:%.2d.%.3d]", m, s, ms);
...
void LrcProcessor::GatherInfoMap(QStringList infoLabels)
{
    ...
    offsetValue = infoMap["offset"].toInt();
}
...
//				m_vecNeteaseLrc.emplace_back(timeInfoLine); //添加一行到网易云歌词
