...
{
    auto pointer = static_cast<VideoPlayer *>(ctx);
...
//    }
    ...
    pFormatCtx->interrupt_callback.callback = CheckInterrupt;
...
//            }
    ...
    while (true)
    {
        ...
        if (seek_req)
        {
            ...
            qDebug()<<" video:"<<pFormatCtx->streams[videoStream]->duration<<" "<<pFormatCtx->streams[videoStream]->time_base.den
                    <<" audio:"<<pFormatCtx->streams[audioStream]->duration<<" "<<pFormatCtx->streams[audioStream]->time_base.den<<'\n';
