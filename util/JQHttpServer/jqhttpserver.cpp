...
void JQHttpServer::Session::replyRedirects(const QUrl &targetUrl, const int &httpStatusCode) {
    ...
    if (QThread::currentThread() != this->thread()) {
        ...
        QMetaObject::invokeMethod(this, "replyRedirects", Qt::QueuedConnection, Q_ARG(QUrl, targetUrl),
                                  Q_ARG(int, httpStatusCode));
        return;
    }
...
void JQHttpServer::Session::replyFile(const QString &fileName, const QByteArray &fileData, const int &httpStatusCode) {
    ...
    if (QThread::currentThread() != this->thread()) {
        ...
        QMetaObject::invokeMethod(this, "replyFile", Qt::QueuedConnection, Q_ARG(QString, fileName),
                                  Q_ARG(QByteArray, fileData), Q_ARG(int, httpStatusCode));
        return;
    }
...
void JQHttpServer::Session::replyImage(const QImage &image, const QString &format, const int &httpStatusCode) {
    ...
    if (QThread::currentThread() != this->thread()) {
        ...
        QMetaObject::invokeMethod(this, "replyImage", Qt::QueuedConnection, Q_ARG(QImage, image),
                                  Q_ARG(QString, format), Q_ARG(int, httpStatusCode));
        return;
    }
...
void JQHttpServer::Session::replyImage(const QString &imageFilePath, const int &httpStatusCode) {
    ...
    if (QThread::currentThread() != this->thread()) {
        ...
        QMetaObject::invokeMethod(this, "replyImage", Qt::QueuedConnection, Q_ARG(QString, imageFilePath),
                                  Q_ARG(int, httpStatusCode));
        return;
    }
...
void JQHttpServer::Session::replyBytes(const QByteArray &bytes, const QString &contentType, const int &httpStatusCode) {
    ...
    if (QThread::currentThread() != this->thread()) {
        ...
        QMetaObject::invokeMethod(this, "replyBytes", Qt::QueuedConnection, Q_ARG(QByteArray, bytes),
                                  Q_ARG(QString, contentType), Q_ARG(int, httpStatusCode));
        return;
    }
        ...
        forever {
            ...
            if (requestMethod_.isEmpty()) {
                ...
                if ((requestMethod_.toUpper() == "GET") ||
                    ...
                    ((requestMethod_.toUpper() == "POST") && ((contentLength_ > 0)
                                                                  ? (!receiveBuffer_.isEmpty())
                                                                  : (true))) ||
                    ((requestMethod_.toUpper() == "PUT") && (
                         (contentLength_ > 0) ? (!receiveBuffer_.isEmpty()) : (true)))) {
...
bool JQHttpServer::AbstractManage::startServerThread() {
    ...
    auto future = QtConcurrent::run(serverThreadPool_.data(), [ &semaphore, this ]() {
...
void JQHttpServer::AbstractManage::newSession(const QPointer<Session> &session) {
    session->setHandleAcceptedCallback([ this ](const QPointer<JQHttpServer::Session> &session) {
        this->handleAccepted(session);
    });
...
void JQHttpServer::AbstractManage::handleAccepted(const QPointer<Session> &session) {
    ...
    auto future = QtConcurrent::run(handleThreadPool_.data(), [ this, session]() {
...
// TcpServerManage
JQHttpServer::TcpServerManage::TcpServerManage(const int &handleMaxThreadCount): AbstractManage(handleMaxThreadCount) {
}
...
JQHttpServer::SslServerManage::SslServerManage(const int &handleMaxThreadCount): AbstractManage(handleMaxThreadCount) {
}
...
// Service
QSharedPointer<JQHttpServer::Service> JQHttpServer::Service::createService(
    const QMap<ServiceConfigEnum, QVariant> &config) {
...
bool JQHttpServer::Service::initialize(const QMap<JQHttpServer::ServiceConfigEnum, QVariant> &config) {
    ...
    if (httpPort > 0) {
        ...
        this->httpServerManage_->setHttpAcceptedCallback(
            std::bind(&JQHttpServer::Service::onSessionAccepted, this, std::placeholders::_1));
    ...
    if (httpsPort > 0) {
        ...
        this->httpsServerManage_->setHttpAcceptedCallback(
            std::bind(&JQHttpServer::Service::onSessionAccepted, this, std::placeholders::_1));
                ...
                case ListVariantMapReceiveDataType: {
                    ...
                    if (!json.isNull()) {
                        QMetaObject::invokeMethod(
                            ...
                            it->slotName.toLatin1().data(),
                            Qt::DirectConnection,
                            Q_ARG(QList<QVariantMap>,
                                  Service::variantListToListVariantMap( json.array().toVariantList() )),
                            Q_ARG(QPointer<JQHttpServer::Session>, session)
                        );
                        return;
                    }
