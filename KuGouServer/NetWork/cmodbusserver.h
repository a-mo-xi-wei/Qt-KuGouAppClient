#ifndef _C_MODBUSSERVER_H_INCLUDE_
#define _C_MODBUSSERVER_H_INCLUDE_
...
public:
    ...
    virtual bool acceptNewConnection(QTcpSocket *newClient);
...
public:
    ...
    /// 服务器是否启动成功
    inline bool isServerStartupSuccessed(void) { return m_isServerStartupSuccessed; }
    /// 设置客户端是否连接成功
    void setIsClientConnected(bool isConn);
    /// 得到客户端是否连接成功
    inline bool isClientConnected(void) { return m_isClientConnected; }
    /// 设置ModbusDataUnitMap
    bool setModbusDataUnitMap(const QModbusDataUnitMap &map);
    /// 得到ModbusDataUnitMap
    inline QModbusDataUnitMap& getModbusDataUnitMap(void) { return m_ModbusDataUnitMap; }
    /// 设置服务器地址
    ...
    /// 服务器是否只侦听
    inline bool isServerListenOnly(void) { return m_isListenOnly;  }
    /// 是否设置服务器忙
    inline bool isServerDeviceBusy(void) { return m_isDeviceBusy; }
