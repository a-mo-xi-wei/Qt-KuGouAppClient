...
public:
    /// 构造函数
    CSVNVersionServer(QObject *parent = nullptr);
    /// 析构函数
    ~CSVNVersionServer();
    ...
    /// 服务器是否启动
    inline bool isListening(void) { return m_webSocketServer.isListening(); }
...
private:
    ...
    /// 添加指定ID保存的版本数据
    bool addVersionData(int id,QByteArray data);
    /// 得到指定ID保存的版本数据
    QByteArray getVersionData(int id);
    /// 检测指定用户ID和密码的用户是否存在
    bool isExistUser(QWebSocket *conn,int id,QString pwd);
    /// 插入一条用户操作日志信息
    void insertuseroperatorlog(int userid,int type,int grade,QString logmsg);
