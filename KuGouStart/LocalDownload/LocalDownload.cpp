      ...
      , m_searchAction(new QAction(this))
{
    ...
    //排序相关
    connect(m_sortOptMenu, &SortOptionMenu::defaultSort, this, &LocalDownload::onDefaultSort);
    connect(m_sortOptMenu, &SortOptionMenu::addTimeSort, this, &LocalDownload::onAddTimeSort);
    connect(m_sortOptMenu, &SortOptionMenu::songNameSort, this, &LocalDownload::onSongNameSort);
    connect(m_sortOptMenu, &SortOptionMenu::singerSort, this, &LocalDownload::onSingerSort);
...
void LocalDownload::MySort(std::function<bool(const MusicItemWidget *, const MusicItemWidget *)> comparator) {
    ...
    for (auto &val: this->m_MusicItemVector) {
        ...
        dynamic_cast<QVBoxLayout *>(ui->local_song_list_widget->layout())->insertWidget(
            ui->local_song_list_widget->layout()->count() - 1, val);
