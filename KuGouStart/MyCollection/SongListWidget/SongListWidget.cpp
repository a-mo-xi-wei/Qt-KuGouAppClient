...
{
    ui->batch_toolButton->setIcon(QIcon(QStringLiteral(":/Res/tabIcon/batch-operation-gray.svg"))); ///< 设置批量操作图标
...
void SongListWidget::initBlock() const
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (i == 0) {
            block->setCoverPix(QStringLiteral(":/Res/tabIcon/like.png")); ///< 设置“我喜欢”封面
        } else {
            block->setCoverPix(QStringLiteral(":/Res/tabIcon/playlist.png")); ///< 设置其他封面
        }
