...
{
    auto collect_download_toolButton_toolTip = new ElaToolTip(ui->collect_download_toolButton); ///< 创建下载按钮工具提示
    collect_download_toolButton_toolTip->setToolTip(QStringLiteral("下载")); ///< 设置下载提示
    auto collect_share_toolButton_toolTip = new ElaToolTip(ui->collect_share_toolButton); ///< 创建分享按钮工具提示
    collect_share_toolButton_toolTip->setToolTip(QStringLiteral("分享"));   ///< 设置分享提示
    auto collect_album_toolButton_toolTip = new ElaToolTip(ui->collect_album_toolButton); ///< 创建专辑按钮工具提示
    collect_album_toolButton_toolTip->setToolTip(QStringLiteral("专辑"));    ///< 设置专辑提示
    auto collect_sort_toolButton_toolTip = new ElaToolTip(ui->collect_sort_toolButton); ///< 创建排序按钮工具提示
    collect_sort_toolButton_toolTip->setToolTip(QStringLiteral("当前排序方式：默认排序")); ///< 设置默认排序提示
    auto collect_batch_toolButton_toolTip = new ElaToolTip(ui->collect_batch_toolButton); ///< 创建批量操作按钮工具提示
    ...
    connect(m_sortOptMenu, &SortOptionMenu::defaultSort, this, [this, collect_sort_toolButton_toolTip](const bool &down) {
        ...
        collect_sort_toolButton_toolTip->setToolTip(QStringLiteral("当前排序方式：默认排序")); ///< 更新提示
    });
    connect(m_sortOptMenu, &SortOptionMenu::addTimeSort, this, [this, collect_sort_toolButton_toolTip](const bool &down) {
        ...
        collect_sort_toolButton_toolTip->setToolTip(down ? QStringLiteral("当前排序方式：添加时间降序") : QStringLiteral("当前排序方式：添加时间升序")); ///< 更新提示
    });
    connect(m_sortOptMenu, &SortOptionMenu::songNameSort, this, [this, collect_sort_toolButton_toolTip](const bool &down) {
        ...
        collect_sort_toolButton_toolTip->setToolTip(down ? QStringLiteral("当前排序方式：歌曲名称降序") : QStringLiteral("当前排序方式：歌曲名称升序")); ///< 更新提示
    });
    connect(m_sortOptMenu, &SortOptionMenu::singerSort, this, [this, collect_sort_toolButton_toolTip](const bool &down) {
        ...
        collect_sort_toolButton_toolTip->setToolTip(down ? QStringLiteral("当前排序方式：歌手降序") : QStringLiteral("当前排序方式：歌手升序")); ///< 更新提示
    });
    connect(m_sortOptMenu, &SortOptionMenu::durationSort, this, [this, collect_sort_toolButton_toolTip](const bool &down) {
        ...
        collect_sort_toolButton_toolTip->setToolTip(down ? QStringLiteral("当前排序方式：时长降序") : QStringLiteral("当前排序方式：时长升序")); ///< 更新提示
    });
    connect(m_sortOptMenu, &SortOptionMenu::playCountSort, this, [this, collect_sort_toolButton_toolTip](const bool &down) {
        ...
        collect_sort_toolButton_toolTip->setToolTip(down ? QStringLiteral("当前排序方式：播放次数降序") : QStringLiteral("当前排序方式：播放次数升序")); ///< 更新提示
    });
    ...
    ui->collect_play_toolButton->setIcon(QIcon(QStringLiteral(":/TabIcon/Res/tabIcon/play3-white.svg"))); ///< 设置播放图标
    ui->collect_download_toolButton->setIcon(QIcon(QStringLiteral(":/TabIcon/Res/tabIcon/download-gray.svg"))); ///< 设置下载图标
    ui->collect_download_toolButton->installEventFilter(this); ///< 安装下载按钮事件过滤器
    this->m_searchAction->setIcon(QIcon(QStringLiteral(":/MenuIcon/Res/menuIcon/search-black.svg"))); ///< 设置搜索图标
...
{
    ElaMessageBar::warning(ElaMessageBarType::BottomRight, "Warning", QString("暂无音乐"), 1000, this->window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "下载 功能暂未实现 敬请期待", 1000, this->window()); ///< 显示提示
}
...
bool SingleSong::eventFilter(QObject *watched, QEvent *event)
{
    if (watched == ui->collect_download_toolButton)
    {
        ...
        {
            ui->collect_download_toolButton->setIcon(QIcon(QStringLiteral(":/MenuIcon/Res/menuIcon/download-blue.svg"))); ///< 设置蓝色下载图标
        }
        else if (event->type() == QEvent::Leave)
        {
            ui->collect_download_toolButton->setIcon(QIcon(QStringLiteral(":/TabIcon/Res/tabIcon/download-gray.svg"))); ///< 设置灰色下载图标
        }
    }
    if (const auto button = qobject_cast<QToolButton *>(watched); button && button->defaultAction() == this->m_searchAction)
    {
        if (event->type() == QEvent::Enter)
        {
            this->m_searchAction->setIcon(QIcon(QStringLiteral(":/MenuIcon/Res/menuIcon/search-blue.svg"))); ///< 设置蓝色搜索图标
        }
        else if (event->type() == QEvent::Leave)
        {
            this->m_searchAction->setIcon(QIcon(QStringLiteral(":/MenuIcon/Res/menuIcon/search-black.svg"))); ///< 设置黑色搜索图标
        }
    }
