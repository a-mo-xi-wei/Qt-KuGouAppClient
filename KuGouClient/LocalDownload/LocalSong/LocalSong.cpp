...
void LocalSong::initUi()
{
    ...
    connect(m_sortOptMenu, &SortOptionMenu::addTimeSort, this, [this, local_sort_toolButton_toolTip](const bool & down)
    {
        ...
        local_sort_toolButton_toolTip->setToolTip(down ?
                ...
                QStringLiteral("当前排序方式：添加时间升序")); ///< 更新提示
        local_sort_toolButton_toolTip->adjustSize();                                    ///< 调整提示大小
    });
    connect(m_sortOptMenu, &SortOptionMenu::songNameSort, this,
            [this, local_sort_toolButton_toolTip](const bool & down)
    {
        onSongNameSort(down); ///< 调用歌曲名称排序
        local_sort_toolButton_toolTip->setToolTip(
            down ? QStringLiteral("当前排序方式：歌曲名称降序") : QStringLiteral("当前排序方式：歌曲名称升序")); ///< 更新提示
        local_sort_toolButton_toolTip->adjustSize();                                   ///< 调整提示大小
    });
    ...
    connect(m_sortOptMenu, &SortOptionMenu::durationSort, this,
            [this, local_sort_toolButton_toolTip](const bool & down)
    {
    ...
    connect(m_sortOptMenu, &SortOptionMenu::playCountSort, this,
            [this, local_sort_toolButton_toolTip](const bool & down)
    {
        onPlayCountSort(down); ///< 调用播放次数排序
        local_sort_toolButton_toolTip->setToolTip(
            down ? QStringLiteral("当前排序方式：播放次数降序") : QStringLiteral("当前排序方式：播放次数升序")); ///< 更新提示
        local_sort_toolButton_toolTip->adjustSize();                                   ///< 调整提示大小
    });
    ...
    ui->local_all_play_toolButton->setIcon(QIcon(QStringLiteral(":/TabIcon/Res/tabIcon/play3-white.svg")));
    ///< 设置播放按钮图标
    ...
    connect(ui->local_search_suggest_box, &ElaSuggestBox::suggestionClicked, this,
...
void LocalSong::getMetaData()
{
    ...
    connect(m_player.get(), &QMediaPlayer::mediaStatusChanged, [ = ](const QMediaPlayer::MediaStatus & status)
    {
        ...
        if (status == QMediaPlayer::LoadedMedia)
        {
            ...
            const auto it = std::find(this->m_locationMusicVector.begin(), this->m_locationMusicVector.end(),
            ...
            if (it == this->m_locationMusicVector.end())
            {
                ...
                ElaMessageBar::success(ElaMessageBarType::BottomRight, "Success",
                                       QString("成功添加音乐 : %1").arg(item->m_information.songName), 500,
                                       this->window()); ///< 显示成功提示
...
void LocalSong::MySort(std::function<bool(const MusicItemWidget*, const MusicItemWidget*)> comparator)
{
    ...
    {
        ElaMessageBar::warning(ElaMessageBarType::BottomRight, "Warning", QString("暂无音乐"), 1000, this->window());
        ///< 显示警告
        return;
    }
...
void LocalSong::on_upload_toolButton_clicked()
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info",
                               QString("%1 功能暂未实现 敬请期待").arg(ui->upload_toolButton->text()), 1000,
                               this->window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "分享 功能暂未实现 敬请期待", 1000, this->window());
    ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "专辑 功能暂未实现 敬请期待", 1000, this->window());
    ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "批量操作 功能暂未实现 敬请期待", 1000, this->window());
    ///< 显示提示
}
...
void LocalSong::onRandomSort()
{
    ...
    std::shuffle(this->m_musicItemVector.begin(), this->m_musicItemVector.end(),
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "Add to play queue not implemented", 1000,
                               window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "Add to new song list not implemented", 1000,
                               window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "Add to loved songs not implemented", 1000,
                               window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "Add to collection not implemented", 1000,
                               window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "Add to playlist not implemented", 1000,
                               window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "Find similar songs not implemented", 1000,
                               window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "View song info not implemented", 1000,
                               window()); ///< 显示提示
}
...
{
    ElaMessageBar::information(ElaMessageBarType::BottomRight, "Info", "Open in file explorer not implemented", 1000,
                               window()); ///< 显示提示
}
