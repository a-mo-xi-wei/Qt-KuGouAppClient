...
{
    QPixmap scaled
        = src.scaled(size, Qt::KeepAspectRatioByExpanding, Qt::SmoothTransformation); ///< 缩放图片
    QPixmap dest(size);
...
void KuGouClient::initUi()
{
    ...
    this->setWindowFlags(
        Qt::FramelessWindowHint | Qt::NoDropShadowWindowHint); ///< 设置无边框和无阴影
    // @note 移动窗口到屏幕中央
    ...
    connect(m_searchResultWidget.get(), &SearchResultWidget::cancelLoopPlay, this, [this] {
        if (m_isSingleCircle)
    ...
    if (id == 3 || id == 16 || id == 11) {
        ...
        if (id == 16) {
            ui->stackedWidget->setCurrentWidget(
                this->m_searchResultWidget.get()); ///< 切换到搜索结果界面
            enableButton(true);
        } else
...
void KuGouClient::initPlayWidget()
{
    ...
    ui->love_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/collect.svg"))); ///< 设置收藏图标
    ui->download_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/download.svg"))); ///< 设置下载图标
    ui->comment_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/comment.svg"))); ///< 设置评论图标
    ui->share_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/share.svg")));                         ///< 设置分享图标
    ui->more_toolButton->setIcon(QIcon(QStringLiteral(":/Res/playbar/more.svg"))); ///< 设置更多图标
    ui->pre_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/previous-song.svg"))); ///< 设置上一首图标
    ui->play_or_pause_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/pause.svg"))); ///< 设置播放/暂停图标
    ui->next_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/next-song.svg"))); ///< 设置下一首图标
    ui->erji_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/together.svg"))); ///< 设置一起听图标
    ui->lyrics_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/song-words.svg"))); ///< 设置歌词图标
    ui->song_queue_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/playbar/play-list.svg"))); ///< 设置播放队列图标

    ui->cover_label->setPixmap(roundedPixmap(
        QPixmap(":/Res/playbar/default-cover-gray.svg"),
        ui->cover_label->size(),
        8));                                   ///< 设置默认封面
    ...
    connect(
        this,
        &KuGouClient::curPlaySongNameChange,
        [this, song_name_text_toolTip](const QString &songName) {
            ...
            if (fm.horizontalAdvance(songName) > 60)
                ui->song_name_text->setFixedWidth(
                    fm.horizontalAdvance(songName) > maxWidth ? maxWidth
            ...
            QString elidedText = fm.elidedText(
                songName, Qt::ElideRight, ui->song_name_text->width()); ///< 计算省略文本
    ...
    connect(
        this,
        &KuGouClient::curPlaySingerChange,
        [this, singer_text_toolTip](const QString &singerName) {
            ...
            if (fm.horizontalAdvance(singerName) > 60)
                ui->singer_text->setFixedWidth(
                    fm.horizontalAdvance(singerName) > maxWidth ? maxWidth
                                                                : fm.horizontalAdvance(singerName));
            else
                ui->singer_text->setFixedWidth(fm.horizontalAdvance(singerName));
            ui->singer_text->setText(
                fm.elidedText(singerName, Qt::ElideRight, ui->singer_text->width())); ///< 设置省略文本
    ...
    connect(
        ...
        this,
        &KuGouClient::updateSliderRange); ///< 连接时长变化信号

    connect(this->m_player, &VideoPlayer::pictureFound, this, [this](const QPixmap &pix) {
        if (pix.isNull()) {
            if (m_player->getMusicPath().startsWith("http://")
                || m_player->getMusicPath().startsWith("https://"))
                return;
            ...
            ui->cover_label->setPixmap(
                roundedPixmap(pix, ui->cover_label->size(), 8)); ///< 设置封面图片
        }
    }); ///< 连接封面图片发现信号
    ...
    connect(this->m_player, &VideoPlayer::audioPlay, this, [this] {
        ui->play_or_pause_toolButton->setIcon(
            QIcon(QStringLiteral(":/Res/playbar/play.svg"))); ///< 设置播放图标
    });                                                       ///< 连接音频播放信号
    connect(this->m_player, &VideoPlayer::audioPause, this, [this] {
        ui->play_or_pause_toolButton->setIcon(
            QIcon(QStringLiteral(":/Res/playbar/pause.svg"))); ///< 设置暂停图标
    });                                                        ///< 连接音频暂停信号
    ...
    connect(this->m_player, &VideoPlayer::errorOccur, this, [this](const QString &msg) {
        ElaMessageBar::error(
            ElaMessageBarType::BottomRight, "Error", msg, 2000, this->window()); ///< 显示错误提示
    });                                                                          ///< 连接错误信号
    ...
    connect(
        ...
        this,
        &KuGouClient::updateProcess); ///< 连接进度条释放信号
}
...
void KuGouClient::initMenu()
{
    ...
    ui->recommend_you_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/recommend.svg"))); ///< 设置推荐图标
    ...
    ui->channel_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/my-channel.svg"))); ///< 设置频道图标
    ...
    ui->ai_chat_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/ai-chat.svg"))); ///< 设置 AI 聊天图标
    ui->song_list_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/song-list.svg"))); ///< 设置歌单图标
    ui->daily_recommend_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/daily.svg"))); ///< 设置每日推荐图标
    ui->my_collection_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/collect.svg"))); ///< 设置收藏图标
    ui->local_download_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/download.svg"))); ///< 设置本地下载图标
    ui->music_cloud_disk_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/cloud.svg"))); ///< 设置云盘图标
    ui->purchased_music_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/bought.svg"))); ///< 设置已购音乐图标
    ui->recently_played_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/window/history.svg"))); ///< 设置最近播放图标
    ui->all_music_toolButton->setIcon(
        QIcon(QStringLiteral(":/Res/titlebar/menu-black.svg"))); ///< 设置全部音乐图标
    // @note 设置互斥按钮组
...
void KuGouClient::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (isPress) {
        if (mouse_press_region == kMousePositionMid) {
            if (ui->title_widget->geometry().contains(m_pressPos)
                || ui->play_widget->geometry().contains(m_pressPos)) {
...
void KuGouClient::resizeEvent(QResizeEvent *event)
{
    ...
    // @note 移动角标
    this->m_sizeGrip->move(
        this->width() - this->m_sizeGrip->width() - 8,
    ...
    if (!this->m_player->getMusicPath().isEmpty()) {
        const QFontMetrics fm(ui->song_name_text->font());
        ui->song_name_text->setText(
            fm.elidedText(m_musicTitle, Qt::ElideRight, ui->song_name_text->width()));
        ///< 设置歌曲省略文本
        ui->singer_text->setText(
            fm.elidedText(m_musicArtist, Qt::ElideRight, ui->singer_text->width())); ///< 设置歌手省略文本
    }
    // @note 同步刷新遮罩大小
    ...
    if (watched == ui->progressSlider) {
        ...
        if (event->type() == QEvent::MouseButtonPress) {
            ...
            if (mouseEvent->button() == Qt::LeftButton) {
                ///< 左键按下
                qint64 value = QStyle::sliderValueFromPosition(
        ...
        if (event->type() == QEvent::Enter) {
            ui->cover_label->setPixmap(roundedPixmap(
                QPixmap(":/Res/playbar/default-cover-blue.svg"),
                ui->cover_label->size(),
                8)); ///< 设置悬停封面
 ...
 * @note 切换搜索结果界面
 */
void KuGouClient::handleSuggestBoxSuggestionClicked(
    const QString &suggestText, const QVariantMap &suggestData)
{
    if (suggestText.isEmpty() || suggestText.trimmed().isEmpty() || suggestText.isNull()) {
        ElaMessageBar::warning(
            ElaMessageBarType::BottomRight, "Warning", "Empty Suggestion", 2000, this->window());
        ///< 显示播放失败提示
        return;
    }
    ///< 之前的还没有加载完成就等候
...
{
    qint64 position = ui->progressSlider->value() * this->m_player->getTotalTime()
                      / ui->progressSlider->maximum();
    ///< 计算播放位置
    // @note 未使用，保留用于调试
        ...
        if (mediaStatusConnection) {
            ...
            mediaStatusConnection = connect(this->m_player, &VideoPlayer::audioFinish, this, [this] {
                ...
                if (ui->stackedWidget->currentWidget()
                        == static_cast<QWidget *>(this->m_localDownload.get())
                    && this->m_localDownload)
                    this->m_localDownload->audioFinished(); ///< 通知本地下载组件
                if (ui->stackedWidget->currentWidget()
                        == static_cast<QWidget *>(this->m_searchResultWidget.get())
