...
void Channel::initUi()
{
    ...
    const auto future = Async::runAsync(QThreadPool::globalInstance(), [this]
    {
        ...
        {
            auto obj = item.toObject();
            this->m_songAndsinger.emplace_back(obj.value(QStringLiteral("song")).toString(),
        ...
        std::shuffle(this->m_pixPathVector.begin(), this->m_pixPathVector.end(),
                     std::default_random_engine(seed)); ///< 随机打乱封面图片
...
{
    const QVector<QPair<QWidget*, QPushButton*>> sectionMappings = {{m_recommendWidget.get(), ui->recommend_pushButton},
