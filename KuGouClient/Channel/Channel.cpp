...
void Channel::initUi()
{
    ...
    }
    {
        ...
        connect(vScrollBar, &QScrollBar::valueChanged, this, &Channel::handleWheelValue); ///< 连接滚动条信号
    }

    // 异步加载 JSON 文本
    const auto future = Async::runAsync(QThreadPool::globalInstance(), [this] {
        QFile file(GET_CURRENT_DIR + QStringLiteral("/title.json")); ///< 加载 JSON 文件
        if (!file.open(QIODevice::ReadOnly)) {
          qWarning() << "Could not open file for reading title.json";
          STREAM_WARN() << "Could not open file for reading title.json"; ///< 记录警告日志
          return true;
        }
        ...
        for (const auto &item: arr) {
          ...
          this->m_titleVector.emplace_back(obj.value(QStringLiteral("title")).toString());
        }
        file.close();
        file.setFileName(GET_CURRENT_DIR + QStringLiteral("/desc.json")); ///< 设置为 desc.json
        if (!file.open(QIODevice::ReadOnly)) {
            qWarning() << "Could not open file for reading desc.json";
            STREAM_WARN() << "Could not open file for reading desc.json"; ///< 记录警告日志
            return true;
        }
        const auto descObj = QJsonDocument::fromJson(file.readAll()); ///< 解析 desc.json
        arr = descObj.array();
        for (const auto &item : arr) {
            auto obj = item.toObject();
            this->m_songAndsinger.emplace_back(obj.value(QStringLiteral("song")).toString(), obj.value(QStringLiteral("singer")).toString());
        }
        ...
        std::shuffle(this->m_titleVector.begin(), this->m_titleVector.end(), std::default_random_engine(seed));
        return true;
    });
...
void Channel::loadSectionBlocks(PartWidget *section, const int &cnt, const int &sum) {
    for (int i = 1; i <= cnt; ++i)
    {
        ...
        block->setSingerSongText(this->m_songAndsinger[i + sum].first + " - " + this->m_songAndsinger[i + sum].second);
...
void Channel::handleWheelValue(const int &value)
{
    const QVector<QPair<QWidget*, QPushButton*>> sectionMappings = {
        ...
        {m_musicalInstrumentWidget.get(),     ui->musical_instrument_pushButton},
        {m_labelWidget.get(),                 ui->label_pushButton},
        {m_varietyWidget.get(),               ui->variety_pushButton},
        {m_nationalCustomsWidget.get(),       ui->national_customs_pushButton},
